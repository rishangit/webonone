...
const KIND = CompanyWebLayoutComponent.KIND_FOOTER;

class CompanyWebFooter {
  constructor(data) {
    this.id = data.id;
  ...
  static async create(data) {
    ...
    return CompanyWebFooter._fromComponent(created);
  }

  static async findById(id) {
    const c = await CompanyWebLayoutComponent.findById(id);
    return CompanyWebFooter._fromComponent(c);
  }

  static async findByCompanyId(companyId) {
    const list = await CompanyWebLayoutComponent.findByCompanyId(companyId, KIND);
    return list.map((c) => CompanyWebFooter._fromComponent(c)).filter(Boolean);
  }
  ...
  static async update(id, data) {
    ...
    return CompanyWebFooter._fromComponent(updated);
  }
...
module.exports = CompanyWebFooter;
