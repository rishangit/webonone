...
async function addGalleryImagesColumn() {
  ...
  try {
    ...
    if (tables.length === 0) {
      console.log('Table company_services does not exist. Skipping migration.');
      return;
    }
    ...
    if (columns.length > 0) {
      console.log('Column galleryImages already exists in company_services table');
      return;
    }
    ...
    console.log('Successfully added galleryImages column to company_services table');
  } catch (error) {
