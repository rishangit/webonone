      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="shrink-0 h-9 w-9" aria-label="Footer actions">
              <MoreVertical className="w-4 h-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-52 border-[var(--glass-border)] bg-[var(--glass-bg)]">
            <DropdownMenuItem className="cursor-pointer" onClick={() => onEditDesigner(footer)}>
              <Pencil className="w-4 h-4 mr-2" />
              Edit with designer
            </DropdownMenuItem>
            {!footer.isDefault && (
              <DropdownMenuItem className="cursor-pointer" onClick={() => onSetDefault(footer)}>
