...
import { AppointmentHistoryItem } from '@/features/appointments/services/appointmentHistory';
...
const appointmentHistorySlice = createSlice({
  ...
  initialState,
  reducers: {
    fetchAppointmentHistoryRequest: (state, _action: PayloadAction<{
      page?: number;
      limit?: number;
      offset?: number;
      search?: string;
    ...
    fetchUserAppointmentHistoryRequest: (state, _action: PayloadAction<{ userId: string; companyId?: string }>) => {
