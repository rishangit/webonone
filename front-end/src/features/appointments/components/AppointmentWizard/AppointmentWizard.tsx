...
import { fetchServicesRequest } from "@/shared/store/services-catalog";
import { fetchStaffRequest } from "@/shared/store/staff";
import { fetchUserRequest, fetchUsersRequest } from "@/shared/store/users";
import { fetchSpacesRequest } from "@/shared/store/spaces";
import { createAppointmentRequest } from "@/features/appointments/store";
import { formatAvatarUrl } from "@/shared/utils";
import { Currency } from "@/shared/services/currencies";
import { fetchCurrencyRequest } from "@/shared/store/sales";
import { CreateUserDialog } from "@/shared/components/users";
...
export function AppointmentWizard({
  ...
  const { staff, loading: staffLoading, error: _staffError } = useAppSelector((state) => state.staff);
                ...
                <div className="space-y-3">
                  ...
                  {(!selectedEntities || selectedEntities.includes('service')) && selectedService && (
                    <div className="space-y-2 p-3 rounded-lg bg-[var(--glass-bg)] border border-[var(--glass-border)]">
                      ...
                      {selectedServiceData && (() => {
                        // Get image URL - handle both image and legacy imageUrl on API payloads
                        const svc = selectedServiceData as { image?: string | null; imageUrl?: string | null };
                        const imageSrc = svc.image || svc.imageUrl;
