...
export const VerifyEmailPage = () => {
  ...
  useEffect(() => {
    ...
    const verifyEmail = async () => {
      try {
        ...
        if (result.success) {
          const needsPassword = "needsPassword" in result && Boolean((result as any).needsPassword);
          if (needsPassword) {
  ...
  const onPasswordSubmit = async (data: { password: string; confirmPassword: string }) => {
    try {
      ...
      const result = await authService.verifyEmailSetPassword(verificationToken, data.password, data.confirmPassword);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:bg-gradient-to-br dark:from-gray-900 dark:via-black dark:to-gray-800 flex items-center justify-center p-2 md:p-4">
      ...
      <div className="w-full max-w-md relative z-10">
          ...
          <div className="text-center space-y-6">
            ...
            {status === "password-setup" && (
              <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} className="space-y-4">
                ...
                <Button type="submit" variant="accent" className="w-full font-medium" disabled={isSettingPassword}>
                  {isSettingPassword ? (
            ...
            {status === "success" && (
              <div className="space-y-3">
                <Button variant="accent" className="w-full font-medium" onClick={() => navigate("/system/dashboard")}>
                  Go to Dashboard
                </Button>
        ...
        <div className="text-center mt-6">
          <p className="text-muted-foreground text-xs">© {new Date().getFullYear()} WebOnOne. All rights reserved.</p>
        </div>
      </div>
    </div>
  );
