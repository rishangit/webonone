...
import { Eye, EyeOff, Lock, Mail, User, UserPlus, ArrowLeft, ArrowRight, Check, X } from "lucide-react";
...
import { signUpRequest } from "@/features/auth/store/authSlice";
  ...
  const onNameSubmit = async (data: { firstName: string; lastName: string }) => {
    ...
    if (isExistingUser && existingUser) {
      try {
        ...
        await authService.setupExistingAccount(existingUser.id, data.firstName, data.lastName, mobileValue || undefined);
      ...
      try {
        ...
        const emailValue = emailForm.getValues("email");
        const mobileValue = mobileForm.getValues("mobileNumber");
        await authService.createUserWithoutPassword(emailValue, data.firstName, data.lastName, mobileValue || undefined);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:bg-gradient-to-br dark:from-gray-900 dark:via-black dark:to-gray-800 flex items-center justify-center p-2 md:p-4">
      ...
      <div className="w-full max-w-lg relative z-10">
        ...
        {/* Progress Bar */}
        <div className="mb-6">
          <ProgressBar value={progress} wrapperClassName="" />
        </div>
        ...
        <Card className="p-8 backdrop-blur-xl bg-[var(--glass-bg)] border border-[var(--glass-border)] shadow-2xl">
          ...
          {currentStep === 1 && (
            <form onSubmit={emailForm.handleSubmit(onEmailSubmit)} className="space-y-4">
              ...
              <Button type="submit" variant="accent" className="w-full font-medium" disabled={isCheckingUser}>
                {isCheckingUser ? (
          ...
          {currentStep === 2 && (
            <form onSubmit={mobileForm.handleSubmit(onMobileSubmit)} className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <Button type="submit" variant="accent" className="flex-1 font-medium" disabled={isCheckingUser}>
                  {isCheckingUser ? (
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                      Checking...
          ...
          {currentStep === 3 && existingUser && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button type="button" variant="outline" onClick={() => handleExistingUserResponse(false)} className="flex-1">
                  <X className="w-4 h-4 mr-2" />
                  No, Not Me
                </Button>
                <Button type="button" variant="accent" onClick={() => handleExistingUserResponse(true)} className="flex-1 font-medium">
          ...
          {currentStep === 4 && (
            <form onSubmit={nameForm.handleSubmit(onNameSubmit)} className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <Button type="submit" variant="accent" className="flex-1 font-medium" disabled={isSettingUpAccount}>
                  {isSettingUpAccount ? (
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                      Sending...
          ...
          {currentStep === 5 && (
            <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <Button type="submit" variant="accent" className="flex-1 font-medium">
          ...
          {currentStep === 6 && (setupComplete || signupComplete) && (
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                  ...
                  {setupComplete && isExistingUser ? (
                    <ul className="space-y-2 text-sm text-muted-foreground">
                      ...
                      <li className="flex items-center gap-2">
                        ...
                        <span>
                          <strong className="text-foreground">Email Verification Link:</strong> Click to verify your email address
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                    ...
                    <ul className="space-y-2 text-sm text-muted-foreground">
                      ...
                      <li className="flex items-center gap-2">
                        <span className="text-blue-500">2.</span>
                        <span>
                          <strong className="text-foreground">Email Verification Link:</strong> Click the link in the email to verify your email
                          address
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        <span className="text-blue-500">3.</span>
                        <span>
                          <strong className="text-foreground">Set Password:</strong> After clicking the verification link, you'll be asked to set your
                          account password
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                  <p className="text-sm text-muted-foreground">
                    <strong className="text-foreground">Note:</strong> If you don't see the {setupComplete && isExistingUser ? "emails" : "email"},
              ...
              <div className="flex gap-3">
                <Button type="button" variant="outline" onClick={() => navigate("/system/login")} className="flex-1">
                  Go to Login
                </Button>
                <Button
                  type="button"
