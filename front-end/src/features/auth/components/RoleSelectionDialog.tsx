...
import { Building2, User, Shield, Briefcase } from "lucide-react";
...
export const RoleSelectionDialog = ({ open, roles, onRoleSelect, onCancel, isLoading = false }: RoleSelectionDialogProps) => {
  const [selectedRoleId, setSelectedRoleId] = useState<string | null>(roles.find((r) => r.isDefault)?.id ?? roles[0]?.id ?? null);
    ...
    <CustomDialog
      ...
      footer={
        <div className="flex gap-3 w-full">
          ...
          <Button onClick={handleSelect} disabled={selectedRoleId === undefined || isLoading} variant="accent" className="flex-1 font-medium">
            {isLoading ? (
