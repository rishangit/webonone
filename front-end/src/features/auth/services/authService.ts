...
import { SignUpFormData, AuthResponse, User, UserRole } from "@/features/auth/types";
...
class AuthService {
  ...
  static async completeLoginWithRole(
    email: string,
    roleId: string | null,
  ): Promise<AuthResponse & { selectedRole?: { role?: number; companyId?: string | null } }> {
    try {
  ...
  static async impersonateUser(
    token: string,
    userId: string,
  ): Promise<(AuthResponse & { originalAdmin: any }) | { requiresRoleSelection: true; user: User; roles: any[]; originalAdmin: any }> {
    try {
  ...
  static async createUserWithoutPassword(
    ...
    lastName: string,
    mobileNumber?: string,
  ): Promise<{ success: boolean; message: string; data: { userId: string; email: string } }> {
    try {
