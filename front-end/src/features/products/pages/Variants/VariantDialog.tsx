...
import { variantSchema, VariantFormData } from "@/features/products/schemas";
  ...
  const generateSKUFromAttributes = async (currentAttributeValues: Record<string, string>) => {
    ...
    try {
      ...
      if (attributeValuesForSKU.length > 0) {
        const { generateVariantSKUFromAttributes } = await import("@/shared/utils/skuGenerator");
        // Try to get product name, fallback to variant name or 'Product'
        let productName = 'Product';
        try {
          const { productsService } = await import("@/features/products/services/productApi");
              ...
              <div className="space-y-2">
                ...
                <SystemProductVariantSelector
                  ...
                  onSystemVariantCreated={(systemVariantId) => {
                    ...
                    import("@/features/products/services/productVariants").then(({ productVariantsService }) => {
