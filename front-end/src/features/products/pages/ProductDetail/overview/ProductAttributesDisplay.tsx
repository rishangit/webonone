...
import type { ProductVariant as SystemProductVariant } from "@/features/products/services/productVariants";
...
export const ProductAttributesDisplay = ({
  ...
  useEffect(() => {
    ...
    Promise.all([
      productRelatedAttributesService.getAttributesByProductId(productId),
  ...
  // Fetch attribute values for every variant in parallel
  useEffect(() => {
    let cancelled = false;
  ...
  const findMatchingVariant = (next: ValueMap): SystemProductVariant | null => {
  ...
  const handlePick = (productRelatedAttributeId: string, value: string) => {
    ...
    // Only push to parent on a match — keeps the previous selection stable on
    // partial / no-match picks while the user is still configuring.
    ...
    <Card className="p-6 backdrop-blur-xl bg-[var(--glass-bg)] border-[var(--glass-border)]">
      ...
      <div className="space-y-3">
        {productAttributes.map((attr, index) => {
          ...
          // A single declared option behaves as a fixed product-level constant.
          const fixedSingleOption = options.length === 1 ? options[0] : "";
          const pickValue = picks[attr.id] ?? "";
          // Prefer per-variant stored value, then fall back to the declared single option.
