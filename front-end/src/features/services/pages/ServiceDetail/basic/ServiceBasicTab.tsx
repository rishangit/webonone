...
interface ServiceBasicTabProps {
  service: Service;
  formatCurrency: (amount: number) => string;
  formatDuration: (minutes: number) => string;
}

export function ServiceBasicTab({ service, formatCurrency, formatDuration }: ServiceBasicTabProps) {
  return (
    <Card className="p-6 backdrop-blur-sm bg-[var(--glass-bg)] border border-[var(--glass-border)] w-full">
      <div className="space-y-4 w-full">
        <div>
          ...
          {service.description && <p className="text-muted-foreground break-words">{service.description}</p>}
        </div>
        ...
        {service.tags && service.tags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            ...
            {service.tags.map((tag, index) => (
              <Badge key={index} variant="outline" className="bg-[var(--glass-bg)] border-[var(--glass-border)]">
