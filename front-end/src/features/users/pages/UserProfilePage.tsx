...
import { fetchUserRequest, clearError } from "@/features/users/store";
import { UserAppointmentHistoryPanel } from "@/shared/components/appointments";
...
export const UserProfilePage = ({ userId, onBack }: UserProfilePageProps) => {
  ...
  const { users: allUsers, currentUser: reduxCurrentUser, loading, error } = useAppSelector(
    (state) => state.users
  );
      ...
      <Card className="p-6 backdrop-blur-xl bg-[var(--glass-bg)] border-[var(--glass-border)] shadow-lg">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          <Avatar className="w-24 h-24">
            <AvatarImage
              src={
                user.avatar ? formatAvatarUrl(user.avatar, user.firstName, user.lastName) : undefined
              }
              alt={fullName}
            />
        ...
        <div className="mt-6">
          {companyId ? (
            <UserAppointmentHistoryPanel
              ...
              userDataOverride={historyUserData}
